      ...
      )
{
   assert(paraInstance);
   ScipParaInstance *scipParaInstance = dynamic_cast<ScipParaInstance*>(paraInstance);
   // nOrgVars = paraInstance->getNVars();           // number of original variables in LC
   int nOrgVarsInSolver = SCIPgetNOrigVars(scip);
   // if( nOrgVars == 0 ) nOrgVars = nOrgVarsInSolver;
   nOrgVars = nOrgVarsInSolver;
   assert( nOrgVars <= paraInstance->getVarIndexRange() );
   orgVarLbs = new SCIP_Real[nOrgVarsInSolver];
   orgVarUbs = new SCIP_Real[nOrgVarsInSolver];
   if( scipParaInstance->isOriginalIndeciesMap() )
   {
            ...
            paraParams->getBoolParamValue(UG::CommunicateTighterBoundsInRacing) )
      {
         ...
         for( int v = 0; v < nOrgVarsInSolver ; v++ )
         {
      ...
      for( int i = 0; i < nOrgVarsInSolver; i++ )
      {
