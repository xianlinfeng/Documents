...
\Begin{
	\eIf{$s^0\text{ is feasible to }\mathcal{P}^1$}{
			\textbf{return} $s^0$\;
 			...
 			\For{$j$ in $\mathcal{I}$} {  
 				$\mathcal{P}^2 \longleftarrow$ create new variables $y_j$ with objective value $\alpha$ and add it to $\mathcal{P}^2$:\; 
 				   \tcp*[r]{add new variables} 
 				$\mathcal{P}^2 \longleftarrow $ add new constraints to $\mathcal{P}^2 \text{ : }y_j \geq x_j - s_j^0$ \tcp*[r]{add new constraints}
 				$\mathcal{P}^2 \longleftarrow $ add new constraints to $\mathcal{P}^2 \text{ : } y_j \geq s_j^0 - x_j$\;
 			...
 			\While{$\alpha > 0 $}{
 				...
 				update the variable $y_j$ coefficient\;
