...
\Begin{
	\eIf{$s^0\text{ is feasible to }\mathcal{P}^1$}{
			\textbf{return} $s^0$\;
 			...
 			$\mathcal{I} \longleftarrow$ index set of integer or binary variables\;
 			\For{$i$ in $\mathcal{I}$} {  
 			...
 			\For{$l$ in $ \{k, k-1,\cdots, 0  \} $}{
 				...
 				change the last $	\mathbf{card}(\mathcal{I})$ coefficients in coefs[\,] $\longleftarrow \alpha $\;
 				\If{$l = 0$}{
 					stop gap $\longleftarrow 0.0$\;
 				}
 				$s' \longleftarrow $ solving the sub-MIP problem to stop criterion\;
